...
- Messaging and turn control:
  ...
  - queue replay no longer drops unknown-type or schema-invalid persisted jobs; recovery preserves them as explicit terminal failures (`recovery_unknown_job_type:*`, `recovery_invalid_payload:*`) for observability and client reconciliation.
  - `file_change_explain` jobs auto-recover stale/missing project orchestration sessions (`thread not found` / invalid mapping) and degrade to synthesized fallback explainability text instead of surfacing raw worker failure errors in chat transcript rows.
  - file-change explainability now enqueues at file-change approval request time (`item/fileChange/requestApproval`), re-reconciles during `GET /api/sessions/:sessionId/approvals` for already-pending approvals, and also enqueues on eligible completed `fileChange` items (deduped by project/thread/turn/item); lifecycle writes synthetic explainability transcript rows anchored to the source diff item.
  - approval-time explainability enqueue now tolerates sparse approval payloads by deriving/falling back missing anchors (`turnId`/`itemId`), hydrating missing diff `changes` from matching supplemental `fileChange` transcript entries when approval params are sparse, and tagging explainability rows with `approvalId` metadata so pending approval cards can still link and render queued/running explainability state.
  - explainability content is intentionally narrow: output is constrained to `What changed` only (concise change-focused bullets), with no rationale/risk/follow-up sections.
  - supervisor queue workflow is now implemented end-to-end with three additional queue job types:
    - `file_change_supervisor_insight` (approval/completion-time diff insight with `change`, `impact`, `risk`, `check`).
    - `risk_recheck_batch` (turn-end batch execution of non-`none` risk checks).
    - `turn_supervisor_review` (turn-level synthesis after risk batch terminal).
  - file-change supervisor insights enqueue during file-change approval request/reconcile and on eligible completed file-change items; lifecycle writes transcript rows (`type: fileChange.supervisorInsight`) anchored to the source diff/approval.
  - turn completion now triggers supervisor post-processing (`enqueueTurnSupervisorPostProcessing`): gather applied non-`none` risks, run one risk-recheck batch per turn when candidates exist, then enqueue a turn supervisor review.
  - risk recheck and turn review lifecycle rows are persisted into transcript as `riskRecheck.batch` and `turn.supervisorReview` with explicit terminal states for UI reconciliation (`complete` / `error` / `canceled`).
  - supervisor risk/review state is persisted across restarts in `.data/supervisor-ledger.json` (per project/turn insight ledger plus batch/review job ids) and flushed on shutdown.
  - suggest-reply prompt construction now includes latest supervisor context (turn review guidance + unresolved risk batch results) when available, so suggested replies incorporate risk-aware forward guidance.
...
- Chat runtime features:
  ...
  - turn-end supervisor outputs render as dedicated thought rows:
    - `riskRecheck.batch` (`Risk Recheck Results`)
