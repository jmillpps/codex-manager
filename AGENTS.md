...
- `apps/api`: Fastify backend supervising `codex app-server` over STDIO with session/project lifecycle APIs, thread/turn control endpoints, approvals/tool-input workflows, queue-backed suggested-request + file-change explainability orchestration jobs, hidden system-owned orchestrator session isolation, metadata persistence (including supplemental transcript ledger and orchestrator job store), websocket fan-out, and structured error mapping. Session-mutating control routes are existence-gated to avoid orphan per-session control metadata writes. Detailed runtime semantics and endpoint behavior contracts are documented in `docs/implementation-status.md`.
