...
- `apps/web`: React + Vite chat UI with ChatGPT-like split-pane layout, project/chat navigation and management flows, consolidated turn rendering (user request + unified response card with progressive thought disclosure), markdown-rendered assistant responses and thought reasoning/message previews (including collapsible bold-title subsections inside expanded thought details), approvals/tool-input inline handling, model/reasoning controls with immediate per-chat persistence that survives app/browser restarts (including stable same-session selection sync, model-list dedupe by id to avoid selection churn, and inline effort chips per model without hover-only flyouts), per-chat approval-policy selector support (`Unless Trusted`/`On Failure`/`On Request`/`Never`), suggest-reply integration, thread actions/insights/settings surfaces, websocket-backed streaming lifecycle UX, disconnected-chat reconnect overlay UX, optimistic outgoing-message delivery-state indicators (`Sending`/`Sent`/`Delivered`/`Failed`), incoming response receive-state indicators (spinner while streaming, disconnect marker on drop), a completion checkmark on final assistant replies, memoized transcript rendering to keep composer typing responsive, and session-switch race guards that prevent stale transcript/approval/tool-input hydration from leaking prior-chat content or duplicating delivered user bubbles. Detailed behavior contracts (including scroll anchoring/snap-back tuning and approval rendering semantics) are documented in `docs/implementation-status.md`.
- `apps/api`: Fastify backend supervising `codex app-server` over STDIO with session/project lifecycle APIs, thread/turn control endpoints, approvals/tool-input workflows, suggested-reply orchestration behavior, metadata persistence (including supplemental transcript ledger), websocket fan-out, and structured error mapping. Detailed runtime semantics and endpoint behavior contracts are documented in `docs/implementation-status.md`.
