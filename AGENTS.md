...
- `apps/web`: React + Vite chat UI with ChatGPT-like split-pane layout, project/chat navigation and management flows, consolidated turn rendering (user request + unified response card with progressive thought disclosure), markdown-rendered assistant responses and thought reasoning/message previews (including collapsible bold-title subsections inside expanded thought details), approvals/tool-input inline handling, model/reasoning controls with immediate per-chat persistence that survives app/browser restarts, suggest-reply integration, thread actions/insights/settings surfaces, websocket-backed streaming lifecycle UX, disconnected-chat reconnect overlay UX, optimistic outgoing-message delivery-state indicators (`Sending`/`Sent`/`Delivered`/`Failed`), incoming response receive-state indicators (spinner while streaming, disconnect marker on drop), a completion checkmark on final assistant replies, and memoized transcript rendering to keep composer typing responsive. Detailed behavior contracts (including scroll anchoring/snap-back tuning and approval rendering semantics) are documented in `docs/implementation-status.md`.
