...
function buildSupervisorBootstrapInstruction(): string {
  return [
    ...
    "- do not run turn steer unless auto-steer is enabled for the job and assessed risk is at or above the configured auto-steer threshold",
...
function buildFileChangeSupervisorReviewText(job: FileChangeSupervisorReviewJob): string {
  ...
  return [
    ...
    'EXPLAIN_FILE=\"$(mktemp \"$WORKSPACE_ROOT/.data/supervisor/explainability.XXXXXX.md\")\"',
    'INSIGHT_FILE=\"$(mktemp \"$WORKSPACE_ROOT/.data/supervisor/insight.XXXXXX.md\")\"',
    'STEER_FILE=\"$(mktemp \"$WORKSPACE_ROOT/.data/supervisor/steer.XXXXXX.md\")\"',
    '```',
    "Use absolute file paths for `--content-file` and `--input-file` arguments.",
    "",
    "Write explainability streaming state first:",
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${explainabilityId} --turn-id ${job.context.turnId} --entry-role system --type fileChange.explainability --status streaming --content "Analyzing proposed file change..." --details '${detailsText}'`,
    '```',
    "",
    ...
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${explainabilityId} --turn-id ${job.context.turnId} --entry-role system --type fileChange.explainability --status complete --content-file \"$EXPLAIN_FILE\" --details '${detailsText}'`,
    '```',
    "",
    "Write supervisor insight streaming state next:",
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${supervisorInsightId} --turn-id ${job.context.turnId} --entry-role system --type fileChange.supervisorInsight --status streaming --content "Supervisor analyzing diff..." --details '${detailsText}'`,
    '```',
    "",
    ...
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${supervisorInsightId} --turn-id ${job.context.turnId} --entry-role system --type fileChange.supervisorInsight --status complete --content-file \"$INSIGHT_FILE\" --details '${detailsText}'`,
    '```',
    "",
...
function buildTurnSupervisorReviewText(job: TurnSupervisorReviewJob): string {
  ...
  return [
    ...
    'WORKSPACE_ROOT="$(pwd)"',
    'mkdir -p .data/supervisor',
    'REVIEW_FILE=\"$(mktemp \"$WORKSPACE_ROOT/.data/supervisor/turn-review.XXXXXX.md\")\"',
    '```',
    "",
    "Set review row to streaming before synthesis:",
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${reviewMessageId} --turn-id ${job.context.turnId} --entry-role system --type turn.supervisorReview --status streaming --content "Supervisor reviewing completed turn..." --details '${detailsText}'`,
    '```',
    "",
    "Write final review text to `$REVIEW_FILE`, then set review row to complete:",
    '```bash',
    `pnpm --filter @repo/cli dev --json sessions transcript upsert --session-id ${job.context.sourceSessionId} --message-id ${reviewMessageId} --turn-id ${job.context.turnId} --entry-role system --type turn.supervisorReview --status complete --content-file \"$REVIEW_FILE\" --details '${detailsText}'`,
    '```'
  ].join("\n");
}
...
function buildSuggestRequestText(job: SuggestRequestJob): string {
  return [
    ...
    "",
    "## Requested Action",
    "Perform this action immediately: draft one concise next user request that the user can send to the coding agent.",
    "Do not wait for additional context and do not defer synthesis.",
    "",
    "## Routing Context",
    `Project: ${job.context.projectId}`,
    `Source session: ${job.context.sourceSessionId}`,
    ...
    valueOrPlaceholder(job.context.turnTranscript),
    "```",
    "The transcript block above is the full chat history context for this suggest-request job (user + assistant flow).",
    "Use this provided context directly and do not perform additional context lookups.",
    "",
    ...
    "Do not run context-retrieval commands (`sessions get`, `sessions inspect`, `stream events`) for this job.",
    "You already have the required history context in this instruction payload.",
    "",
